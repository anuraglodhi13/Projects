...
import org.nagarro.facade.NotificationApp;
...
public class NotificationSystemAppFacade {
    public void main () {
        System.out.println("Implementing using Facade Design Pattern");
        ...
        switch(channelIndex) {
            ...
            case 2: { // send Sms
                ...
                for(User user : users) {
                    if(user.getIsSubscribed()) {
                        notification.sendSms(subject, body);
                        System.out.println("Sent to user "+ user.getName());
                    }
                }
                break;
